...
parser.add_option("--LooseCRe3g0","--looseCRe3g0", dest="isLooseCRe3g0Selection",default=False,action="store_true",
                  help="Use exactly 3j and 0btag control region selection")
...
#LooseCR3e0Selection = options.isLooseCR3e0Selection
...
if makeEGammaPlots:
	prefit = " Combine_MisIDEle_looseCRe3g0_barrel.root"
	...
	if finalState=="mu":
		...
		if LooseCRe3g1Selection:
			_extratext="=3jets,>=1btag"
                        ...
                        _extratext="=3jets,0btag"
                        ext="e3jet0tag"
                        prefit = "Combine_ZJets_ele_e3jets0b.root"
		else:
			_extratext=">=4jets,>=1btag"
                        ext="signal"
...
observables_AxisTitles = {"M3_control":"M3 GeV",
			  "M3":"M3 (GeV)",
...
for obs in observables:
 ...
 #       print postfitFile, "%s_postfit/%s"%(obs,template)	
	    ...
	    #print template, hist, postFitHist
...
#		legend.AddEntry(data,"Data",'pe')
		...
		legend1.AddEntry(sig_,"MisIdentified Electron",'lp')
	...
	#	sum_.Draw('hist,same')
                ...
                sum_.GetYaxis().SetTitle("Events/10GeV")
...
#		bkg_.Scale(1.,"width")
	...
	#		sum_.GetXaxis().SetTitle("(e^{+},e^{-}) invariant mass (GeV)")
		...
		sum_.GetYaxis().SetTitleSize(gStyle.GetTitleSize()/(1.-padRatio+padOverlap))
		sum_.GetYaxis().SetTitleOffset(gStyle.GetTitleYOffset()*(1.-padRatio+padOverlap))
		sum_.GetYaxis().SetTitle("<Events/GeV>")
		sig_.Draw("same,hist")
		bkg_.Draw("same,hist")
		data.Draw('E,X0,SAME')
		legend1.Draw("same")
		...
		axisTitle = obs
		if obs in observables_AxisTitles:
			axisTitle = observables_AxisTitles[obs]
